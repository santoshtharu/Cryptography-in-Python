...
def makeDiagraph(text):
    ...
    return list
print(makeDiagraph(addFillerLetter(PT)))
...
def generate5x5Matrix(list1, key):
    keylist = []
    matrixlist = []
    matrix = []
    for i in key:
...
print(np.array(matrix).reshape(5,5))
...
def columnRule(matrix, e1r, e1c, e2r, e2c):
    ...
    if e2r ==4:
        ...
        c2 = matrix[e2r+1][e2c]
    return c1, c2
print(columnRule(matrix, 0, 4, 4, 4))
...
def encryptionUsingPlayfair(matrix, plaintextlist):
    ciphertextlist = []
    c1, c2 = 0, 0
...
print(''.join(encryptionUsingPlayfair(matrix, plaintextlist)))
